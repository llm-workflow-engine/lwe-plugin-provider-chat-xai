...
with open(path.join(FILE_DIR, 'lwe_plugin_provider_chat_xai', 'version.py')) as f:
    version = re.match(r'^__version__ = "([\w\.]+)"$', f.read().strip())[1]

setup(
    name="lwe-plugin-provider-chat-xai",
    version=version,
    author="Chad Phillips",
    author_email="chad@apartmentlines.com",
    description="LWE plugin: Chat xAI Provider plugin",
    ...
    entry_points={
        "lwe_plugins": [
            "lwe_plugin_provider_chat_xai = lwe_plugin_provider_chat_xai.plugin:ProviderChatXai"
        ]
    },
)
